                ...
                <tbody>
                 <?php 
                  if (class_exists('EmployerClass')) {
                    ...
                    if (method_exists($em, 'getBoardOfDirectors')) {
                      $result = $em->getBoardOfDirectors('senior_management_team');
                      if ($result) {
                        $i = 0;
                        while ($rows = $result->fetch_assoc()) {
                          $i++;

                       $img = "../images/senior_management_team/".$rows['em_id']."/".$rows['em_photo'];
                        if (!file_exists($img)) {
                           $img = "../img/logo.png";
                         }
                 ?>
                 <tr>
                   ...
                   <td><a class="btn btn-info onclick_data" data-bodvid="<?php echo $rows['em_id']; ?>" data-toggle="modal" data-target="#bodDetails" href="#"><i class="fa fa-eye"></i> View </a>
                    <a href="senior_management_team_edit.php?bodeid=<?php echo $rows['em_id']; ?>" class="btn btn-warning"><i class="fa fa-edit"></i> Edit</a>
...
<script>
  $(document).ready(function(){
     $(document).on('click', '.onclic_del_bod', (function() {
         ...
         var em_type = "senior_management_team";
          $.ajax({
                type: "post",
