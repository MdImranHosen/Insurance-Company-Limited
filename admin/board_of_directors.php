                ...
                <tbody>
                 <?php 
                  if (class_exists('EmployerClass')) {
                    ...
                    if (method_exists($em, 'getBoardOfDirectors')) {
                      $result = $em->getBoardOfDirectors('board_of_directors');
                      if ($result) {
                        $i = 0;
                        while ($rows = $result->fetch_assoc()) {
                          $i++;

                        $img = "../images/board_of_directors/".$rows['em_id']."/".$rows['em_photo'];
                        if (!file_exists($img)) {
                           $img = "../img/logo.png";
                         }
                 ?>
                 <tr>
                   ...
                   <td><a class="btn btn-info onclick_data" data-bodvid="<?php echo $rows['em_id']; ?>" data-toggle="modal" data-target="#bodDetails" href="#"><i class="fa fa-eye"></i> View </a>
                    <a href="board_of_directors_edit.php?bodeid=<?php echo $rows['em_id']; ?>" class="btn btn-warning"><i class="fa fa-edit"></i> Edit</a>
...
<script>
  $(document).ready(function(){
     $(document).on('click', '.onclic_del_bod', (function() {
         ...
         var em_type = "board_of_directors";
          $.ajax({
                type: "post",
