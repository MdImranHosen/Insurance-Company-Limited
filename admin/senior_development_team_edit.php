...
if($_GET["bodeid"]) {
   ...
   $bodeid = preg_replace('/\D/', '', $_GET['bodeid']);
   $bodeid = htmlentities($bodeid);

 if (!isset($bodeid) || $bodeid == NULL) {
    header('Location:senior_development_team.php');
  }elseif($bodeid==0){
    header('Location:senior_development_team.php');
  }else{
   $bodeid = (int)$bodeid;
...
if (isset($_POST['save_bod_data'])) {
   ...
   $em_type  = "senior_development_team";
     ...
     <?php 
      if (class_exists('EmployerClass')) {
        ...
        if (method_exists($em, 'boardOfDirectorsUpdateById')) {
          $result = $em->boardOfDirectorsUpdateById('senior_development_team',$bodeid);
         if ($result) {
        ...
        while ($rows = $result->fetch_assoc()) {
          ...
          $image    = "../images/senior_development_team/".$id."/".$logo;
