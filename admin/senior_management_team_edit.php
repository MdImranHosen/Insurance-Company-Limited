...
if($_GET["bodeid"]) {
 ...
 if (!isset($bodeid) || $bodeid == NULL) {
    header('Location:senior_management_team.php');
  }elseif($bodeid==0){
    header('Location:senior_management_team.php');
  }else{
   $bodeid = (int)$bodeid;
...
if (isset($_POST['save_bod_data'])) {
   ...
   $em_type  = "senior_management_team";
     ...
     <?php 
      if (class_exists('EmployerClass')) {
        ...
        if (method_exists($em, 'boardOfDirectorsUpdateById')) {
          $result = $em->boardOfDirectorsUpdateById('senior_management_team',$bodeid);
         if ($result) {
        ...
        while ($rows = $result->fetch_assoc()) {
          ...
          $image  = "../images/senior_management_team/".$id."/".$logo;
