...
if($_GET["bodeid"]) {
 ...
 if (!isset($bodeid) || $bodeid == NULL) {
    header('Location:board_of_directors.php');
  }elseif($bodeid==0){
    header('Location:board_of_directors.php');
  }else{
   $bodeid = (int)$bodeid;
     ...
     <?php 
      if (class_exists('EmployerClass')) {
        ...
        if (method_exists($em, 'boardOfDirectorsUpdateById')) {
          $result = $em->boardOfDirectorsUpdateById('board_of_directors',$bodeid);
         if ($result) {
        ...
        while ($rows = $result->fetch_assoc()) {
          ...
          $image    = "../images/board_of_directors/".$id."/".$logo;
