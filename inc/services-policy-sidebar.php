...
<div class="sidebar-widget categories">
    <h4 class="sidebar-title"><?php if (!empty($page_name)) { echo $page_name; } ?> Policies</h4>
    <div class="widget-content">
        <!-- Blog Category -->
        <ul class="blog-categories">
<?php
if (method_exists($ps, 'getpsPolicyByServices')) {
    $psresult = $ps->getpsPolicyByServices($page_url);
       if ($psresult) {
        while ($psrows = $psresult->fetch_assoc()) {
          ...
          echo '<li '.$active.'><a class="whoareyoumenu" href="'.$psurl_sb.'"><i class="fa fa-angle-right" aria-hidden="true"></i>'.$policy_name_sb.'</a></li>';
        } } }  ?>
...
<?php include_once 'services_support.php'; ?>
</div>